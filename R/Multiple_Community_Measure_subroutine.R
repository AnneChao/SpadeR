...
print.spadeMult <- function(x, ...){
  if(x$datatype=="abundance"){
    ...
    cat('        C1');cat(N);cat('=U1');cat(N);cat(' (q=1)                  ',sprintf("%.4f",temp[2,1]),'     ',sprintf("%.4f",temp[2,2]),'     ',sprintf("%.4f",temp[2,3]),'     ',sprintf("%.4f",temp[2,4]),'\n\n')
    cat('        C2');cat(N);cat(' (Morisita-Horn)            ',sprintf("%.4f",temp[3,1]),'     ',sprintf("%.4f",temp[3,2]),'     ',sprintf("%.4f",temp[3,3]),'     ',sprintf("%.4f",temp[3,4]),'\n')
    cat('        U2');cat(N);cat(' (Regional overlap)         ',sprintf("%.4f",temp[4,1]),'     ',sprintf("%.4f",temp[4,2]),'     ',sprintf("%.4f",temp[4,3]),'     ',sprintf("%.4f",temp[4,4]),'\n\n')
    ...
    if(q == 0 | method=="relative"){
      ...
      ###################################################UqN_ Equal weight
      if(q==0 || q==2){
        cat('    ----------------------Measure U');cat(q);cat('2-----------------------\n\n')
        cat('    Estimator','     Estimate','     s.e.','       95% Confidence Interval\n\n')
        Cqn_PC <- x$pairwise$UqN_ew
        no.temp=1
        for(i in 1:(N-1))
        {
          for(j in (i+1):N)
        ...
        cat('    Average pairwise similarity =',sprintf("%.3f",mean(Cqn_PC[,1])),'\n\n')
        cat('    Pairwise similarity matrix: \n\n')
        C_SM=x$similarity.matrix$UqN_ew
        
        if(q==0){cat('    U02(i,j)  ')}
        if(q==1 & x$method=="relative"){cat('    U12(i,j)  ')}
        if(q==1 & x$method=="absolute"){cat('    U12(i,j)  ')}
        if(q==2){cat('    U22(i,j)  ')}
        for(i in 1:N)
        {
          cat(i,"      ")
    ...
    if(q!=0 & method=="absolute"){
      ...
      cat('    Average pairwise similarity =',sprintf("%.3f",mean(Cqn_PC[,1])),'\n\n')
      cat('    Pairwise similarity matrix: \n\n')
      C_SM=x$similarity.matrix$CqN_ee
      ...
      cat('\n')
      ###################################################UqN_ Equal Effort
      if(q==2){cat('    ----------------------Measure U');cat(q);cat('2-----------------------\n\n')}
      if(q==1)cat('    ----------------------Measure Horn-----------------------\n\n')
      cat('    Estimator','     Estimate','     s.e.','       95% Confidence Interval\n\n')
      Cqn_PC <- x$pairwise$UqN_ee
      no.temp=1
      for(i in 1:(N-1))
      {
        for(j in (i+1):N)
      ...
      cat('    Average pairwise similarity =',sprintf("%.3f",mean(Cqn_PC[,1])),'\n\n')
      cat('    Pairwise similarity matrix: \n\n')
      C_SM=x$similarity.matrix$UqN_ee
      
      if(q==0){cat('    U02(i,j)  ')}
      if(q==1 & x$method=="relative"){cat('    U12(i,j)   ')}
      if(q==1 & x$method=="absolute"){cat('    Horn(i,j) ')}
      if(q==2){cat('    U22(i,j)  ')}
      for(i in 1:N)
      {
        cat(i,"      ")
      ...
      #{
      #  cat('\n')
      #  cat('    ESTIMATION OF MORISITA DISSIMILARITY IN',N,'COMMUNITIES\n\n')
      #  cat('    Estimator','     Estimate','     s.e.','    95% Confidence Interval\n\n')
      #  cat('    1 - C2')
