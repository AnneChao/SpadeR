print.ChaoShared <- function(x, ...){
	...
	if(nrow(x[[2]])==4){
	  ...
	  cat("     \"Whole\" Shared Species Group:", "\n")
	  cat("         Some Statistics:", "\n")
	  cat("         ---------------------------------------------------------------------------", "\n")
    ...
    cat("     \"Rare\" Shared Species Group: (Both Frequencies can only up to 10)", "\n")
      ...
      cat("     Some Statistics:", "\n")
      cat("         --------------------------------------------------------------------------", "\n")
      cat("         Q[11] =", x[[1]]$Q11, "; ", "Q[1+] =", x[[1]]$Q1.plus, ";", "Q[+1] =", x[[1]]$Qplus.1, "; ", "Q[2+] =", x[[1]]$Q2.plus, "; ", "Q[+2] =", x[[1]]$Qplus.2, ";" , "Q[11] =", x[[1]]$Q22, "\n")
      cat("         --------------------------------------------------------------------------", "\n")
...
Chao1-shared: A lower bound of shared species richness derived by Pan et al. (2009).
...
Chao2-shared: An extension of the Chao2 estimator to two communities. It provides a lower bound of shared species richness. See Eq. (3.6) of Pan et al. (2009). It is replaced by Chao2-shared-bc for the case Q[2+]=0 or Q[+2]=0.

Chao2-shared-bc: A bias-corrected form of Chao2-shared. See Eq. (3.7) of Pan et al. (2009).
	')}
}
